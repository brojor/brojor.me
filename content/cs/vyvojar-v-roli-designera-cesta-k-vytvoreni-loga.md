...
Při dynamickém nastavování `font-family` přes `:style` direktivu je důležité u každého písma v objektu font options nastavit vlastnost `global` na hodnotu `true`. Tento krok zajistí správné aplikování fontu v celé aplikaci.
...
Další krokem bylo zvýraznění prvních tří písmen loga (Bro), což posiluje rozpoznatelnost značky a vytváří zajímavý vizuální akcent.
